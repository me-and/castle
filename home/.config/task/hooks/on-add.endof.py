...
if __name__ == '__main__':
    ...
    for time_field in ('due', 'until'):
        if time_field in new_task:
            # Convert the JSON time to local time
            datetime_str = calc_datetime(new_task[time_field])
            date_str, time_str = datetime_str.split('T')

            if time_field == 'due' and time_str == '00:00:00':
                # If the task is due at midnight, pull it back to 23:59:59 the day
                # before.
                new_task[time_field] = calc_datetime(f'{datetime_str} - PT1S')
                messages.append(f'Changed {time_field} from {datetime_str} to {new_task[time_field]}')
